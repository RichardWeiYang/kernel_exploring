...
前者就是查看对应ds_queue->split_queue_len，后者则需要执行关键的操作split_folio()。

## 添加到deferred_list

函数deferred_split_folio()将folio添加到对应的deferred_list中。这样在deferred_split_scan的时候才会扫描到对应的folio，然后继续做拆分。

目前有三种情况会调用deferred_split_folio()
...
当系统有内存压力时，reclaim程序会通过shrinker扫描对应的obj进行回收。但是在deferred_list上的对象不是所有都要拆分，还需要判断一下：

  * partially_mapped 一定会拆分
  * 不是的话，需要用thp_underused()来判断一下

partially_mapped意思是这个大页已经有部分去掉了映射。到这里，需要着重看一下什么是partially_mapped.

## 什么是partially_mapped

一个大页第一次分配的时候是全部映射的，但是用户可能会unmap掉其中的部分。如果这个大页中有一部分没有任何进程映射，那么就称之为partially。

这个的判断在__folio_remove_rmap()函数中完成, 并且在folio的flags上设置partially_mapped。注意：只有匿名页会有这个标识.
