...
每一个系统中能用的内存都要有一个page struct对应,对新加入的内存也不例外。这个步骤是在populate_section_memmap()中完成.

一般情况下，最后会落到vmemmap_populate_basepages(). 并且在没有指定altmap的情况下， page struct是从页分配器中分配出来的。

不过有意思的是，在SPARSEMEM_VMEMMAP的情况下，section_mem_map中的mem_map值应该是没有用到吧？因为__page_to_pfn/__pfn_to_page已经不从这里拿值了。
...
memory_block_online(mem)
    zone = zone_for_pfn_range(mem->online_type, nid, pfn, nr_pages)
    online_pages(pfn, nr_pages, zone, mem->group)
        move_pfn_range_to_zone(zone, pfn, nr_pages, NULL, isolate =true)
            init_currently_empty_zone()
            resize_zone_range(zone, start_pfn, nr_pages)
            resize_pgdat_range(pgdat, start_pfn, nr_pages)
            memmap_init_range(..., isolate)
                __init_single_page()               --- (1)
                __SetPageOffline()
                init_pageblock_migratetype(, isolate)
        online_pages_range()
            generic_online_page()
                __free_pages_core()                --- (2)
                    __ClearPageOffline()
...
* (1) 初始化page struct
* (2) 将page加入到buddy system, isolated
...
在热插拔过程中，有一个不是太清晰的地方就是**新插入的内存算在哪个Zone上**。

这里现有两个大类：

  * auto-online: 取决于/sys/devices/system/memory/auto_online_blocks
  * manually-online: 取决于 echo 到 memory block设备state文件的内容

首先我们在函数add_memory_resource()中看到，会根据mhp_get_default_online()来判断是否马上要online内存。如果没有配置自动上线内存，则可以通过下面小节的“手动上线内存”来上线。

我们可以在文件/sys/devices/system/memory/auto_online_blocks查看默认配置。
...
也可以写入online,online_kernel,online_movable来更改默认操作。

不论是哪种方法，最后都会把目标zone写道mem->online_type中。但是者还没有完，在memory_block_online中，online_pages之前，还要**通过函数zone_for_pfn_range(online_type)再计算**真正的zone。

具体的逻辑可以看zone_for_pfn_range()，这里得到一个重要的结论是 **Zone Normal和Zone Movable可以重叠**。甚至可以间隔添加memory block device到normal和movable zone.

关键的是，这打破了初始化时free_area_init()是不会构造存在重叠的zone的.
...
比如这个/proc/zoneinfo的结果是交替将连续四个memory block设备上线到movable/normal的结果。可以看到，其中显示的spanned都比managed要大不少。
